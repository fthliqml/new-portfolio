  ...
  return (
    <fieldset>
      <div className="flex items-end justify-between border-b border-border pb-3">
        ...
        <button type="button" disabled={readOnly} onClick={() => onChange([...items, { id: crypto.randomUUID(), text: "" }])} className="inline-flex min-h-9 items-center gap-2 border border-border px-3 font-mono text-[0.58rem] uppercase tracking-[0.12em] hover:bg-accent disabled:opacity-40"><Plus className="size-3.5" /> Add</button>
      </div>
      <div className="mt-4 space-y-3">
        {items.map((item, index) => (
          <div key={item.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
            ...
            <div className="flex self-start"><button type="button" disabled={readOnly || index === 0} onClick={() => move(index, -1)} className="size-9 border border-border disabled:opacity-30" aria-label={`Move ${label} ${index + 1} up`}>↑</button><button type="button" disabled={readOnly || index === items.length - 1} onClick={() => move(index, 1)} className="size-9 border border-border disabled:opacity-30" aria-label={`Move ${label} ${index + 1} down`}>↓</button><button type="button" disabled={readOnly || (required && items.length === 1)} onClick={() => onChange(items.filter((current) => current.id !== item.id))} className="grid size-9 place-items-center border border-border disabled:opacity-30" aria-label={`Remove ${label} ${index + 1}`}><Trash2 className="size-3.5" /></button></div>
          </div>
        ))}
      </div>
    </fieldset>
