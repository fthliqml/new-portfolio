...
export function ProjectForm({ project, options, readOnly = false }: ProjectFormProps) {
  ...
  return <form action={formAction} autoComplete="off" onChangeCapture={markDirty} onClickCapture={markDirtyOnButtonClick} onSubmit={markClean} className="space-y-10">
    {state.error && <p ref={errorRef} tabIndex={-1} role="alert" aria-live="assertive" className="border border-destructive/40 bg-destructive/5 px-4 py-3 text-sm text-destructive outline-none focus-visible:ring-2 focus-visible:ring-destructive/30">{state.error}</p>}
